function DimensionSwitch_PRP_Practice
%==========================================================================
% PURPOSE:
...
EXC_ST.RESPCUT=5;%at least n responses
...
%FILE PATH SETTING
FPATH_ST.TXTCODE='dsPRP_PR';
FPATH_ST.EXFILE='DimensionSwitch_PRP_Practice.m';
...
for p=1:length(PARAM_ST.PARAMETER)
    ...
    switch fn
        ...
        case {'CONTEXT'}
            %TASK RULE PAIRS
            singleblock = TASK_ST.TASKPAIRS(:,1) == TASK_ST.TASKPAIRS(:,2);
            taskpairs = RandSample(find(singleblock), [numTB, 1]);
...
%**************************************************************************
global EXC_ST TASK_ST FPATH_ST STIM_ST RECORD_ST TIME_ST COLOR_ST KEY_ST W
global PARAM_ST BLOCK TRIAL BLOCKSTS
%**************************************************************************
...
switch msgtime
    case {'BINTRO'}% Begining Block BREAK screen
         ...
         tasksets = reshape(TASK_ST.TASK',1,[]);%'Low-High','Even-Odd','Red-Blue','Bold-Faint'
...
%Image File Names
%1=filename, 2=imagedata, 3=loaded texture object
